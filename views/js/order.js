   ...
   $('#complete_order').click(function(){
	  ...
	  // alert(user_name + " " +user_address + " " +user_phone + " " +modified_quantity.toString());
	   ...
	   if((modified_quantity.length!=0 || modified_combo_quantity.length!=0) && user_address!="" && user_phone!="" && user_pincode!="" && user_state!="" && user_city)
	   {
		   ...
		   type:"POST",
		   url:"/post_order",
		   data:{user_name:user_name,user_address:user_address,user_phone:user_phone,modified_quantity:modified_quantity,payment_type:payment_type,order_source:order_source,user_note:user_note,user_pincode:user_pincode,user_state:user_state,user_city:user_city,user_email:user_email,total_discount:total_discount,promo_code_id:promo_code_id,modified_combo_quantity:modified_combo_quantity,shipping_cost,promo_code_id:promo_code_id,discount_percentage:discount_percentage},
		   success:function(data){
			   if(data==1)
			   {
			   ...
			   {
				   alert("Some of your ordered product have stock 0. Cannot complete the order!");
			   }
		   }
	       })
	   }
...
function order_quantity(val,product_id,original_quantity,price,unit_price,balance,on_order)
{
	...
	if(found_or_not!=-1)
	{
		...
		modified_quantity[found_or_not].stock=balance;
		modified_quantity[found_or_not].on_order=on_order;
	}
	else
	{
		//alert("Not found");
		modified_quantity.push({"product_id":product_id,"jars":val,"quantity_ordered":quantity_ordered,"amount":amount,unit_price:unit_price,stock:balance,on_order:on_order});
	}
