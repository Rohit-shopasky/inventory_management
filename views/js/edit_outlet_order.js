   ...
   $('#complete_order').click(function(){
	  ...
	  // alert(user_name + " " +user_address + " " +user_phone + " " +modified_quantity.toString());
	   ...
	   if((modified_quantity.length!=0 || modified_combo_quantity.length!=0) && billing_address!="" && phone!="" && pincode!="" && state!="" && city!="")
	   {
	        ...
	        $.ajax({
		   type:"POST",
		   url:"/edit_outlet_order",
		   data:{outlet_name:outlet_name,billing_address:billing_address,pincode:pincode,state:state,city:city,phone:phone,order_source:order_source,outlet_note:outlet_note,email:email,commision:commision,shipping_cost:shipping_cost,gst_no:gst_no,modified_quantity:modified_quantity,modified_combo_quantity:modified_combo_quantity,order_id:order_id},
		   success:function(data){
			   if(data==1)
			   {
	 ...
	 success:function(data){
		 for(var i=0;i<data.length;i++)
		 {
			...
			document.getElementById(id).value=data[i].jars;
             modified_quantity.push({"product_id":data[i].product_id,"jars":"-1","quantity_ordered":data[i].jars,"amount":data[i].amount});		  
...
$.ajax({
	type:"GET",
	url:"/already_outlet_ordered_combo_quantity",
	data:{order_id:order_id},
	success:function(data){
  ...
  $.ajax({
	  type:"GET",
	  url:"/get_outlet_customer_details",
	  data:{order_id:order_id},
	  success:function(data){
		  document.getElementById("outlet_name").value = data[0].outlet_name;      $('#outlet_name').focus();
		  document.getElementById("billing_address").value=data[0].billing_address; $('#billing_address').focus();
		  document.getElementById("user_pincode").value=data[0].pincode;           $('#user_pincode').focus();
		  document.getElementById("states").value=data[0].state;
		  $('<option>').val(data[0].city).text(data[0].city).appendTo('#cities');
		  document.getElementById("cities").value=data[0].city;
		  
		  document.getElementById("user_phone").value = data[0].phone;          $('#user_phone').focus();
          document.getElementById("commision").value = data[0].commision;       $('#commision').focus();
		  document.getElementById("gst_no").value    = data[0].gst_no;          $('#gst_no').focus();
...
function order_quantity(val,product_id,original_quantity,price,all_jars)
{  
	...
	{
		//alert("Not found");
		modified_quantity.push({"product_id":product_id,"jars":val,"quantity_ordered":quantity_ordered,"amount":amount,updated_no_of_jars:updated_no_of_jars});
	}
